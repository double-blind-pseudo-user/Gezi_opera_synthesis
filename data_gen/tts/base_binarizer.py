...
class BaseBinarizer:
    ...
    def process_data(self, prefix):
        ...
        args = []
        builder = IndexedDatasetBuilder(f'{data_dir}/{prefix}') # 创建.data和.idx文件
        lengths = []
        f0s = []
        total_sec = 0
        ...
        builder.finalize()
        np.save(f'{data_dir}/{prefix}_lengths.npy', lengths) # 写入lengths.npy文件
        if len(f0s) > 0:
            f0s = np.concatenate(f0s, 0)
            f0s = f0s[f0s != 0]
            np.save(f'{data_dir}/{prefix}_f0s_mean_std.npy', [np.mean(f0s).item(), np.std(f0s).item()]) # 写入f0s_mean_std.npy
